...
results_dir = "Results"
save_path = os.path.join(results_dir, "base_truce_synthetic.pkl")
# Change working directory
os.chdir(base_dir)

folder_path = os.path.join(base_dir, "DataSet", "Truce_Synthetic","raw")
...
def process_json_file(file_path):
    ...
    df["image_path"] = df["id"].apply(lambda x: os.path.join(base_dir, "DataSet", "Truce_Synthetic", "images",  x))
    ...
    exp_df['annotations'] = exp_df['annotations'].apply(lambda x: x[0].strip().lower())
...
def get_synthetic_data(folder_path,results_dir,save_path):
...
df3 = get_synthetic_data(folder_path,results_dir,save_path)
